...
운영자가 ReplyGuard에 접속하면 모든 관리자 화면에서 동일한 사이드바와 상단 영역을 사용한다. 첫 화면인 대시보드에서는 문의량, 자동 처리, 확인 필요, 처리 실패와 최근 추이·문의 유형·확인 필요 문의를 빠르게 파악할 수 있어야 한다.
...
- **Test scenarios:**
  - KPI가 전체 48건, 자동 발송 24건, 확인 필요 8건, 처리 실패 3건으로 표시된다.
